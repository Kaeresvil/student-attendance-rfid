        ...
        <div class="card">
            <div class="card-body">
                ...
                <!-- <table class="table table-hover table-sm">
                    ...
                    <tbody>
                        <tr v-for="(grade, index ) in gradelevel" :key="gradelevel.id">
            ...
            <a-row style="margin: 15px 15px" v-if="true">
                ...
                <div>
                    <span
                    >{{ gradelevel.from }} - {{ gradelevel.to }} of
                    {{ gradelevel.total }}</span
                    >
                </div>
                <div>
                    <a-pagination
                    size="small"
                    :total="gradelevel.total"
                    show-size-changer
                    @change="onChange"
...
setup(){
    ...
    const columns = [
        ...
        const index = (payload = {page: 1}) => {
            loading.value = true;
                   axios.get('/api/page/grade_level', {params: {...payload}})
                    .then(response => {
                        gradelevel.value = response.data.data;
                        loading.value = false;
                    })
    ...
    const editRecord = (record) => {
            router.push({path: '/edit/grades/' + record.id,
            query: {archive: 'false'}
            })
        }
    onMounted(index)
